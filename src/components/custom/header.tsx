...
export const selectItems = [
  { key: "cheesy-delight", label: "Cheesy Delight" },
  { key: "pizza-hut", label: "Pizza Hut" },
  { key: "kids-corner", label: "Kids Corner" },
];

const Header = () => {
  return (
    <header className="bg-white">
      <nav className="container py-5 mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <Select className="w-[150px]" size="sm">
            {selectItems.map((selectItem) => (
              <SelectItem key={selectItem.key}>{selectItem.label}</SelectItem>
            ))}
          </Select>
        </div>
        <div className="flex items-center space-x-4">
          <ul className="flex items-center font-md gap-4">
            <li>
          ...
          <div className="flex items-center gap-2 ml-12">
            <Phone />
            <span>01712211443</span>
          </div>
          <Button className="bg-orange-500 text-white" radius="sm" size="sm">
            Log Out
          </Button>
        </div>
      </nav>
